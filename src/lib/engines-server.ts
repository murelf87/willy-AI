...
export const PROVIDERS: Provider[] = [
  ...
  { id: "openrouter", name: "OpenRouter (modelos gratuitos)", baseUrl: "https://openrouter.ai/api/v1", keyUrl: "https://openrouter.ai/keys", dataNote: "Solo se usan modelos marcados «:free». Cada proveedor de esos modelos tiene sus propias condiciones.", onlyModelSuffix: ":free" },
  ...
  // NVIDIA Build (NIM): gratis y sin tarjeta, unas 40 peticiones por minuto, con decenas de modelos grandes (DeepSeek, Mistral Large, GLM, Llama…).
  {
    ...
    prefer: ["deepseek-v4[.\\d]*-flash", "mistral-large", "llama-3\\.3-70b-instruct", "glm-5", "deepseek-v4"],
  },
];
...
export type EngineState = { key?: string; enabled: boolean; model?: string; cooldownUntil: number; reason: string; day: string; used: number; lastOk: number };
...
export type FailureKind = "quota-minute" | "quota-day" | "auth" | "payment" | "model" | "transient" | "empty" | "other";
...
export function classifyFailure(status: number, body: string, retryAfterHeader: string | null, now: number): Failure {
  ...
  if ((status === 404 || status === 400) && /model/.test(text) && /(not found|does not exist|unsupported|decommission|deprecat|not available|invalid model|no longer)/.test(text)) {
...
function scoreModel(provider: ProviderId, id: string): number {
  ...
  if (/(mini|nano|lite|small|tiny|instant|1b|3b|8b)/.test(name)) score -= 2;
...
async function markFailure(dir: string, id: string, failure: Failure, now: number, clearModel: boolean): Promise<void> {
  const state = await loadState(dir);
  const engine = state.engines[id];
  if (!engine) return;
  engine.cooldownUntil = failure.cooldownMs > 0 ? now + failure.cooldownMs : 0;
  engine.reason = failure.message;
  if (clearModel) delete engine.model;
  await saveState(dir, state);
}

/** Una petición de conversación a un motor. Aplica las esperas y los límites, y anota el resultado. */
export async function callEngine(env: Env, id: string, messages: ChatMessage[], opts: { maxTokens?: number; temperature?: number } = {}): Promise<CallResult> {
  ...
  if (!model) {
    ...
    const picked = pickModel(provider, listed.ids);
    if (!picked) {
  ...
  const wanted = Math.min(opts.maxTokens ?? 6000, provider.maxOutput ?? MAX_OUTPUT_TOKENS);
  const request = async (maxTokens: number) => {
    try {
      const res = await fetchImpl(`${provider.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({ model, messages, max_tokens: maxTokens, temperature: opts.temperature ?? 0.2, stream: false }),
  ...
  await markFailure(env.dir, id, failure, now, failure.kind === "model");
...
export async function engineAction(dir: string, body: Body, env: Partial<Env> = {}): Promise<Record<string, unknown>> {
  ...
  if (action === "cloud-chat") {
    ...
    const result = await callEngine(full, id, messages, { maxTokens, ...(temperature !== undefined ? { temperature } : {}) });
    return { ...result };
  }
