...
- **SUPER WILLY: estado del proyecto en la cabecera** (local 26): el mismo `projectStatus`/`STATUS_LABEL` que en Proyectos («En desarrollo», «Esperando tu decisión»…). No inventar otra etiqueta de estado.
- **Centro de Inteligencia → Resumen** (local 27): reproduce la maqueta con datos reales. `useRoutingNow()` y `GlobalPrefsCard` son compartidos por Resumen y Routing: si cambia el enrutado, se cambia ahí (y en `routing-table.ts`), no en dos sitios. «Modelos en uso reciente» lee el último mensaje de cada chat (`by`) y las respuestas de SUPER WILLY (`worklog`).
- **Chats** (locales 28 y 29): `saveThread` no guarda si la conversación no ha cambiado (abrirla ya no la sube a «Recientes»); y hay «ideas para empezar» encima del cuadro (solo escriben el principio de la petición, no envían).
- **Traductor: lista de modelos** (local 30): solo ofrece modelos que saben conversar (`isChatModel` de `local-ai.ts`); `nomic-embed-text` y los demás de búsqueda ya no salen. Las demás pantallas que pasan `available` a `runTask`/`planChain` ya filtraban dentro del orquestador; el Centro de Inteligencia → Modelos los sigue enseñando todos a propósito (es el gestor de modelos).
- **Errores de tipos que venían de antes** (local 31): arreglados los 5 (dos `Button variant="default"` que salían SIN estilo en Pruebas y en las tarjetas de Proyectos → `primary`; `return void` en `routeRequest` de `capability-registry.ts`; `MIME["svg"]` en `project-compile-ts.ts`). Desde local 31 el TYPECHECK de la Autoconstrucción dice «Sin errores de tipos»: cualquier error nuevo es de la mejora que se esté instalando.
- **Documentación** (local 32): páginas nuevas «Chats», «Centro de Inteligencia» y «Autoconstrucción». Si cambia cómo funciona una de esas pantallas, se actualiza su página en `DOCS` (`app-sections.tsx`).
- **Chats: «Copiar respuesta»** (local 33) junto a «Leer en voz alta» en cada respuesta de WILLY (usa `copyText` de `workspace-store.ts`). Los pulgares 👍/👎 de la maqueta NO se han puesto: su comportamiento no está definido (pregunta abierta en `claude/decisiones-pendientes-antonio.md`).
- **Textos al día** (local 34): la nota de «IA Influencer» (Herramientas) dice lo que ya funciona (el busto que habla de «Mi yo en IA → Crear vídeo»); «Doblar vídeos» cita también «Transcribir audio o vídeo»; la instalación manual de «Mi yo en IA» ya no habla de Piper cuando solo hay voces naturales (con Piper, el texto de siempre).
- **Diagnóstico de la gráfica** (local 35): `systemStats` (`system-ops.ts`) mira, como mucho una vez por minuto, si a la instalación de Ollama le falta `ggml-cuda.dll` en `lib\ollama\cuda_vNN` (campo `ollamaCudaMissing` de la lectura del equipo) y `gpuUsage` (`system-info.ts`) lo explica así en vez de culpar al controlador de NVIDIA. Lo usan el Centro de Inteligencia, el diagnóstico de Ajustes y los consejos de Inicio. Cuando Antonio reinstale Ollama, el aviso desaparece solo.
- **Plan de SÚPER IA** (local 36): 3 tareas hechas más en el hito «Diseño del dueño» (cabecera de Súper IA, Resumen del Centro de Inteligencia, ideas y «Copiar respuesta» en Chats). Tareas: 53 hechas, 3 pendientes (6 agentes, mascota, IA Influencer/Vídeo IA). Los ids `sN` del plan son correlativos y el plan del sistema es de solo lectura: añadir tareas en medio no rompe nada guardado.
- **Ollama con la gráfica (25/09, ~15:05 UTC)**: Antonio reinstaló Ollama (0.34.4). `server.log`: `inference compute … library=CUDA … GTX 1660 Ti … libdirs=ollama,cuda_v13`. `llama3.2:3b` entero en la gráfica (2.436 MB de VRAM) a ~85 tokens/s; `/api/sistema?ollama` → `usage.state = grafica` y `ollamaCudaMissing` ya no sale. La ventana de Ollama enseña «Couldn't load integrations» en su página «Apps» (lista de programas externos que baja de internet): no afecta al servidor ni a WILLY.
- **Gráfica compartida** (local 37): ahora que Ollama calcula con la gráfica, «Mi yo en IA → Crear vídeo» saca de la gráfica el modelo de Ollama antes de animar con ComfyUI (`unloadOllamaFromGpu` de `voces-chatterbox.ts`, el mismo que usan Chatterbox y Transcribir a través de `makeRoomOnGpu`) y lo dice en los pasos del trabajo. Ollama lo vuelve a cargar solo en la siguiente pregunta. Si se añade otro uso pesado de la gráfica, hay que hacer lo mismo: con 6 GB no caben a la vez.

### Locales 38–49 (25/09/2026, 17:27–18:46 UTC · sesión de claude.ai «WILLY AI»)
- 38 `engines-server.ts`/`chat-cloud.ts`: fallo `too-large` sin espera; cada motor aprende su tope por petición (`maxRequest`); lo que no cabe no se envía.
- 39 `answer-check.ts` (nuevo), `chat-cloud.ts` (`accept`), `superia-view.tsx`, `routing-table.ts`: relevo por calidad (razonamiento en vez de respuesta, archivos rotos, página que no se ejecuta, los mismos archivos cuando se pedía un cambio).
- 40 `ai-standard.ts` (reglas 10 y 11), `project-compile.ts`, `project-workshop.tsx`, `preview-runtime.ts`: proyectos React sin `vista-previa.html`, rutas en código, sin imágenes externas.
- 41 arreglo del 39: construir puede empezar por las direcciones visuales (sin archivos).
- 42 `plan-evidence.ts` (nuevo) + `project-plan-sync.ts`: lo «hecho» por la IA se comprueba en los archivos.
- 43 `project-discovery.ts` (`splitAdditions`): paréntesis no se parten; «sin …» va a las notas. 44 `removeFeature` + botón «Quitar» en la entrevista.
- 45 `engines-server.ts`: modelo saturado → otro modelo del mismo proveedor (`alt`, 6 h); orden de modelos corregido («gemini» contenía «mini»).
- 46 `engines-server.ts`: registro `datos-privados/motores-registro.jsonl` (acción `engines-log`); nunca la clave ni el texto.
- 47 `preview-runtime.ts` (vigía): página que enseña un error («Something went wrong», «X is not defined», «The above error occurred in…») = rota; `superia-view.tsx`: el cuadro del chat del proyecto se vacía al enviar.
- 48 `requestedPages` (`project-discovery.ts`), `DiscoveryInput.pages`, `missingRequestedPages` (`plan-evidence.ts`), `completeRequested` (`superia-view.tsx`): las páginas enumeradas en la idea son tareas comprobables; las que falten se piden una vez.
- 49 `superia-view.tsx`: «Construir ya sin preguntas» crea siempre el proyecto (`ExecOptions.newProject`).
- Regla nueva para quien despliegue: NO desplegar mientras SUPER WILLY esté construyendo (el reinicio mata la petición en curso y se pierde la respuesta). Comprobar `{action:"self-status"}.running` y que no haya un «trabajando…» en la pantalla.
- 50 `answer-check.ts` (`cutDelivery`), `super-willy-projects.ts` (`SavedAnswer.cut/pending`), `ai-standard.ts` (formato, regla 4), `superia-view.tsx` (`continueAttempt`): entrega cortada → no se guarda a medias y WILLY pide que siga (≤3).
- 51 `superia-view.tsx`: continuación con 9.000 caracteres de archivos; tarea «General» sobre un proyecto = web/código. `project-discovery.ts` (`nounPhrase`): nombre sin «en»/«a» al final.
- 52 `project-workshop.tsx` (`VisualContext.resources`), `superia-view.tsx` (`resourcesFixedFor`): imágenes inexistentes → «Arreglo de recursos» automático (1 vez por proyecto).
- 53 `engines-server.ts` (`setAlt`): tiempo agotado → la siguiente petición va a otro modelo del proveedor.
- 54 `engines-server.ts` (`callEngine(opts.compact)`, acción `cloud-chat` con `compact`), `engines-client.ts`, `chat-cloud.ts` (`compact`), `superia-view.tsx` (`filesCompact`): petición compacta si el motor rechaza la grande por saturación.
- 55 `plan-evidence.ts` (`requestFileHints`), `answer-check.ts` (`requiredTexts`, `mustContain`), `superia-view.tsx` (`CHANGE_SCOPE_RULE`, `focus`): pistas de archivo por página nombrada; alcance explícito; el texto pedido entre comillas debe estar en los archivos.
- 56 `preview-runtime.ts` (vigía + `repairHints`): «Not Found» y pushState/replaceState en about:srcdoc = página rota, con pista de `createHashHistory()`.
- 57 `preview-runtime.ts` (`watchAfterSave` sin salto si ya rota): toda edición dispara vigía. `plan-evidence.ts` (`requestFileHints`): quita texto entre comillas antes de emparejar. `answer-check.ts` (`unusableAnswer.mustContain` sólo mira `src/`/`index.html`).
- 58 `worklog.ts` (`WorkTurn.kind:"descartada"`, `historyOf` filtra `!t.kind`): turno descartado no se reenvía a la IA. `chat-cloud.ts` (`onRefused`): aviso visible «🚫 Descartada…».
- 59 `local-fit.ts` (nuevo, `fitsInGpu`/`preferFitting`/`readFitInfo`/`fitNote`): en 6 GB, qwen2.5-coder:7b va a medias con el procesador (4 tok/s); SUPER WILLY prioriza el modelo local que cabe entero.
- 60: prueba real de Autoconstrucción — «modelo(s) en tu equipo» → «modelos en tu equipo» (`superia-view.tsx`), OK en 3 min.
- 61: prueba real de Autoconstrucción — contador singular/plural del modelo local (`superia-view.tsx`), OK en 2 min, sin usar el motor local lento.
- 62 `engine-plan.ts` (`attemptStep` descuenta `skipped.size`): con Gemini caído, la rueda ya no salta motores ni cae antes de tiempo en el modelo local. `capabilities.ts` (`fastCoderFirst` + `preferFitting`/`cachedFitInfo`): Autoconstrucción también prioriza el modelo que cabe en la gráfica. `engines-server.ts` (`EngineState.bad`, `rememberBad`): un modelo con 404 «Not found for account» o que contesta vacío no se vuelve a elegir; OpenRouter prioriza modelos que programan (nemotron-3-super quedó descartado por contestar vacío).
- 63 `orchestrator.ts` (`runTask`, último modelo de la cadena que se niega: reintento con aviso «no hay nada dañino» y si sigue negándose, `fail()` en vez de `ok()`), `translate-flow.ts` (`translateChunks`: una negativa nunca se muestra como traducción, aunque el fragmento sea corto): el traductor de YouTube ya no enseña «Lo siento, pero no puedo…» cuando las palabras del vídeo (p.ej. una mala transcripción automática) disparaban el filtro del último modelo disponible. Probado en vivo con el vídeo real que lo reproducía: ya traduce «Gay Police» como nombre propio sin negarse.

## AVATAR AI: nitidez de caras en «Mi yo en IA → Crear vídeo» (26/09/2026)

Diagnóstico (ver `claude/auditoria-ia-influencer-2026-09-25.md`): el vídeo de ejemplo de Antonio salía borroso, con las gafas con un tono morado y los dientes deformados. Causa: Wav2Lip solo trabaja sobre un recorte de boca de 96×96 px y lo vuelve a pegar sin mejorar la nitidez; LivePortrait tiene dificultad conocida con superficies reflectantes (gafas) — esto último no tiene arreglo de software sencillo, solo probar sin gafas o con otro ángulo.

Hecho (nodo de restauración de cara tras Wav2Lip, antes de guardar el vídeo):
- Clonado `mav-rik/facerestore_cf` en `ComfyUI\custom_nodes\facerestore_cf\` (nodo `FaceRestoreCFWithModel`, con parámetro de fidelidad de CodeFormer).
- Modelos descargados en `ComfyUI\models\facerestore_models\`: `GFPGANv1.4.pth` (240 MB) y `codeformer.pth` (359 MB), completos y verificados por tamaño.
- `flujos\Wav2Lip_labios_API.json` editado: nodos nuevos `5` (`FaceRestoreModelLoader`, carga `codeformer.pth`) y `6` (`FaceRestoreCFWithModel`, `codeformer_fidelity: 0.5`, detector `retinaface_resnet50`) insertados entre Wav2Lip (nodo `3`) y `VHS_VideoCombine` (nodo `4`, que ahora toma `images` de `6` en vez de `3`; el audio sigue saliendo de `3`). Copia de seguridad del flujo original en `flujos\backup-2026-09-26\Wav2Lip_labios_API.json.bak`.
- **Pendiente de Antonio (no lo puede hacer esta sesión: hace falta ejecutar Python de Windows, y el terminal solo se puede pulsar, no escribir en él)**: doble clic en `INSTALAR_MEJORA_CARAS.bat` (nuevo, en la raíz de `WILLY_AVATAR`, mismo patrón que `INSTALAR_OLLAMA_GPU.bat`) para instalar las librerías Python que pide `facerestore_cf` (opencv-python, lpips, gdown…) dentro del venv de ComfyUI. Después, arrancar ComfyUI y generar un vídeo de prueba con «Mi yo en IA → Crear vídeo» para comparar nitidez antes/después.
- No probado con generación real (ComfyUI estaba apagado y no se puede arrancar/operar sin permisos de escritura en un terminal desde esta sesión): el flujo se ha validado solo como JSON válido y con los nodos/parámetros que pide el propio código de `facerestore_cf`.

## AVATAR AI: mejora de caras instalada y confirmada (26/09/2026, tarde)

`INSTALAR_MEJORA_CARAS.bat` ejecutado con éxito por Antonio (tras corregir que lo había lanzado desde la copia descargada del chat en vez de desde `C:\WILLY_AVATAR`: el .bat calcula la ruta de ComfyUI con `%~dp0`, así que solo funciona ejecutado desde su sitio). Librerías de `facerestore_cf` instaladas en el venv de ComfyUI. Pendiente: que Antonio arranque ComfyUI y genere un vídeo de prueba con Wav2Lip para comparar nitidez con el vídeo anterior (gafas moradas, dientes deformados).

## 26/09/2026 16:24–16:30 UTC — local 64: el traductor usa la IA externa primero

Antonio: «el traductor necesito que sea la IA externa quien traduzca, porque la local, no esta sincronizada y si pones en silencio el video no se escucha la traducción».

`translate-view.tsx` traducía los subtítulos de YouTube SOLO con `runTask` (motor local Ollama). El modelo local deja más fragmentos sin traducir o incompletos que Gemini/Mistral, y esas líneas de la voz sincronizada quedan mudas o desincronizadas al silenciar el vídeo.

Cambio: antes de traducir, se llama una vez a `engineStatus()`; si el interruptor «IA externa» está encendido y no en modo ahorro, cada fragmento se traduce con `cloudChat` en el orden de `KIND_CLOUD_ORDER.traduccion` (Gemini, Mistral, Cohere, NVIDIA, Groq, OpenRouter); si todas fallan o están apagadas, sigue con `runTask` (tu equipo) como antes. Los datos sensibles (DNI, IBAN, tarjeta, claves) se siguen quedando siempre en tu equipo (`looksSensitive`).

**local 64**, PASS en build/tipos/pruebas/health. Probado en vivo con un vídeo real y un spy en `fetch`: confirma llamadas a `engines-status` y `cloud-chat` (Gemini) por cada fragmento.

## 26/09/2026 18:18–18:42 UTC — locales 66–70: falso positivo de `validateContent` corregido de verdad + Autoconstrucción más intuitiva

Antonio: «cambia la forma de autoconstrucción hazla diferente mas intuitiva, y 100% efectiva. pruebalo que realmente construye su propio codigo y se automejora» — y antes: «TIENES QUE MEJORARME AUTOCONSTRUCCIÓN, NO TIENES QUE INTERVENIR TU, DEBE HACERLO AUTONOMAMENTE LA PAGINA». Todo lo de abajo se hizo por la pantalla «Autoconstrucción → Nueva mejora» (nunca por `/api/self-build` a mano): la IA de WILLY escribe el SEARCH/REPLACE, decide, prueba e instala sola.

- **66** (descartado tras leer el código instalado, no cuenta como bueno): `patch-apply.ts` `validateContent` pasó de un marcador suelto a los tres, pero además envolvió `INVALID_CODE`/`INVALID_JSON`/`INCOMPLETE_CODE`/`INCOMPLETE_FILE` en un `JSON.stringify({code,message})` no pedido. El TEST_SUCCESS del diario NO lo detectó: hubo que leer el archivo realmente desplegado para verlo.
- **67**: mismo archivo, instrucción más estricta (no tocar el envoltorio del 66): `hasOpen && hasSep && hasClose` en vez de un marcador suelto. Confirmado leyendo el archivo desplegado.
- **68** (el arreglo real y definitivo): `validateContent` ya no mira si HAY un marcador — cuenta bloques `SEARCH/=======/REPLACE` completos y sin resolver con `countBlocks()` y compara `countBlocks(content) > countBlocks(previous)` (mismo patrón que la comparación `PLACEHOLDER` de la misma función, unas líneas más abajo). Así un archivo que ya trae un ejemplo de 3 marcadores como documentación (como `self-build-runner.ts` y `project-work.ts`) deja de estar bloqueado para siempre.
- **69**: primera edición real de `self-build-runner.ts` en toda la vida del proyecto (antes: SIEMPRE rechazado, ver «local 62» más arriba y `claude/willy-coordinacion-cambios-en-equipo.md`). Con el 68 ya puesto: `MAX_ATTEMPTS` 4 → 6. Confirmado con `grep`/`wc -l` sobre el archivo instalado: una sola línea cambiada, nada más.
- **70**: `code-focus.ts` `focusContext` — un archivo adjuntado o nombrado por su ruta completa (`src/...`) de ≤30.000 caracteres se entrega ENTERO a la IA (antes: solo «reforzado» dentro de fragmentos elegidos por palabras clave, que a veces no traían la línea exacta a cambiar). Acertado a la primera (intento 1 de 8, Gemini 3.8 Flash, 1 min 7 s). Confirmado leyendo el archivo instalado línea a línea.

`project-work.ts` tiene el mismo patrón que `self-build-runner.ts` (ejemplo de 3 marcadores dentro de `PROJECT_WORK_RULES`) y por la misma lógica del 68 debería estar igual de desbloqueado, pero no se ha probado todavía con un despliegue real sobre ese archivo — pendiente de confirmar.

Detalle completo, horas exactas y tabla de verificación fila por fila: `claude/willy-coordinacion-cambios-en-equipo.md` (proyecto de claude.ai «WILLY AI»).

## 26/09/2026 ~20:00–20:10 UTC — locales 24–70 subidos a GitHub

`murelf87/willy-ai` (`main`) estaba parado en el commit `da21ccd` (hasta local 23, subido el 25/09). Empaquetado el código real del PC (`src/`, `public/`, `package.json`/configs, `willy.nsi`+lanzadores, `supervisor/`, `COORDINACION-IAS.md`, `mejoras-de-la-web.md`, `maquetas-dueno/`, `willy-release.json`; sin `datos-privados/`, `backups/`, `copias-*/`, `node_modules/`, `.exe`), sustituido en el clon, comprobado con `npm install` + `tsc --noEmit` (0 errores) + `vite build --config vite.config.local.ts` (build real completo, sin errores) y subido: commit **`db4f81a`** (`da21ccd..db4f81a`), 50 archivos, incluye los locales 24–70 y los archivos de «Crea tu avatar IA» (`avatar-character-view.tsx`, `model-install.ts`, etc., aún sin local numerado — no instalados todavía por Autoconstrucción, escritos directamente en el PC). `package-lock.json` regenerado por `npm install` (deriva menor de resolución de dependencias, sin cambio de paquetes). Sin claves ni datos privados (comprobado antes de subir).
